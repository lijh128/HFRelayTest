...
namespace RelayTest.Application.Task
{
    public partial class FrmTaskManager : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void BindGridview()
        {
            ...
            if (dsTestInfo != null)
            {
                for (int i = 0; i < dsTestInfo.Tables[0].Rows.Count; i++)
                {
                    ...
                    lstTasklist.Items.Add(temptaskNo + "_" + allocation + "_" + submit);
                }
            }
        }
        ...
        {
            gcSampleInfoList.Enabled = true;
            btnDelSampleLine.Enabled = true;
            btnNewSampleLine.Enabled = true;

            //清除原有试验任务数据显示
            initData();
            //演示时使用，填写试验任务信息的默认值，正式运行时，注释
            FillBaseInfo();
            //初始化试验任务基础信息
            //试验任务编号，自动生成任务编号
            txtTaskId.Text = createNewTaskId();
            dateEdit_task.Text=string.Format("{0:yyyy/MM/dd}", DateTime.Now);  
        ...
        private void lstTasklist_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (lstTasklist.SelectedIndex >= 0)
            {
                ...
                selecttaskinfo = tempstr;

                nowTaskId = tempstr.Substring(0, tempstr.IndexOf("_"));
                ...
                //如果为已分配样品任务，样品表不可修改，新增行、删除行按钮不可用
                if (tempstr.IndexOf("已分配") > 0)
                {
                    gcSampleInfoList.Enabled = false;
                    btnNewSampleLine.Enabled = false;
                    btnDelSampleLine.Enabled = false;
                }
                else
                {
                    gcSampleInfoList.Enabled = true;
        ...
        {
            BllTEST_INFO blltest_info = new BllTEST_INFO();
            DataSet dsTestInfo = blltest_info.GetListByTaskId(taskId);

            if (dsTestInfo != null)
            {
                //试验任务编号
                txtTaskId.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_TASK_ID"].ToString();
                //检测项目
                txtTestItem.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_ITEM"].ToString();
                //规格型号
                txtTestModel.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_MODEL"].ToString();
                //检测标准
                txtTestStandard.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_STANDARD"].ToString();
                //检测性质
                txtTestProperty.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_PROPERTY"].ToString();
                //检测目的
                txtTestAim.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_AIM"].ToString();
                //委托单位
                txtDeputeUnit.Text = dsTestInfo.Tables[0].Rows[0]["F_TASK_DEPUTE_UNIT"].ToString();
                //联系人
                txtContact.Text = dsTestInfo.Tables[0].Rows[0]["F_CONTACT"].ToString();
                //电话
                txtPhone.Text = dsTestInfo.Tables[0].Rows[0]["F_PHONE"].ToString();
                //email
                txtEmail.Text = dsTestInfo.Tables[0].Rows[0]["F_EMAIL"].ToString();
                //负责人
                txtCharge.Text = dsTestInfo.Tables[0].Rows[0]["F_TEST_CHARGE"].ToString();
                //委托日期
                dateEdit_task.Text = string.Format("{0:yyyy/MM/dd}", Convert.ToDateTime(dsTestInfo.Tables[0].Rows[0]["F_TASK_DEPUTE_DATE"].ToString()));
            }
        }
        ...
        private void displayTaskSampleInfoByTaskId(string taskId)
        {
            ...
            //DataSet _TestSampleTable = bllTestSampleInfo.GetList("F_TEST_TASK_ID='" + now_taskid + "'");
            _TestSampleTable = bllTestSampleInfo.GetAndDisplaySampleInfo(_TestSampleTable,taskId );
